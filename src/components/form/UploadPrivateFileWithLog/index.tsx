 ...
 * @version 1.0.0
 */
const UploadPrivateFileWithLog: React.FC<UploadMultipleImageProps> = (props) => {
  const {
    name,
    label,
    disabled,
  ...
  return (
    <>
      <ProFormUploadButton
        ...
        disabled={disabled}
        listType={type}
        action={`${API_URL}/third-service/${version}/third/file/privatefileUploadWithLog`}
        onChange={(info) => {
          ...
          if (info.file?.status === 'done') {
            ...
            setComplete(!completeFlag);
          }
        }}
        fieldProps={{
          showUploadList: false, // 不显示上传后按钮下的文字
          data: extraParams,
          headers: {
            //@ts-ignore
...
export { UploadPrivateFileWithLog };
