...
import { fetchInvestList } from '@/services/member/list/invest';
 ...
 */
export const handleInvestList = async (params: RequestTableParam) => {
  const hide = message.loading('正在获取数据');

  try {
    const result = await fetchInvestList(params);
